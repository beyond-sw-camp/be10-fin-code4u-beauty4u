...
### 채팅방 생성
![채팅방 생성](/images/채팅방 생성.gif)

### 채팅방 생성시 사용자 조회
![채팅방 사용자 조회](/images/채팅방 사용자 조회.gif)

### 채팅방 사용자 목록 조회
![사용자 목록 조회](/images/사용자 목록 조회.gif)

### 채팅방 사용자 초대
![채팅방 사용자 초대](/images/채팅방 사용자 초대.gif)

### 채팅방 나가기
![채팅방 나가기](/images/채팅방 나가기.gif)

### 채팅방 이미지 파일 전송
![채팅방 이미지 파일 전송](/images/채팅방 이미지 파일 전송.gif)

### 채팅방 이미지 저장
![채팅방 이미지 저장](/images/채팅방 이미지 저장.gif)
</details>
...
| 신민철 | 이전 프로젝트에서 HTTPS 통신 기반의 REST API를 사용한 CRUD 기능을 구현하다가 이번 최종 프로젝트에서 채팅 기능을 담당하게 되었습니다. 웹소켓 통신이 무엇인지 간략한 개념만 알고 있던 상황에서 실시간 채팅 기능을 구현하려고 하니 ERD 설계부터 코드 구현까지낯선 부분이 많았습니다. 단순히 웹소켓 연결만 하면 될 것이라 생각했지만채팅 기능을 구현하면서 생각보다 신경 쓸 것이 많다는 사실을 알게 되었고,그 과정에서 발생하는 트러블 슈팅을 처리하면서원래 목표로 했던 채팅 고도화 기능인 카프카를 설정만 해두고결국 사용하지 못한 것이 아쉽습니다. 그래도 세부적인 기능들을 구현하면서 네트워크 통신이 어떻게 이루어지는지 많이 생각해 볼 수 있는 좋은 시간이었다고 생각합니다. 8주 가량의 길다면 길고 짧다면 짧은 프로젝트지만, 좋은 팀원들이 있어 무사히 마무리를 할 수 있었습니다. 팀원들 모두에게 고맙습니다! |
